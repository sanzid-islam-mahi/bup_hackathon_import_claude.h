...
def _fallback_plan(
    ...
    if abs(drift) > 1e-4:
        ...
        if drift > 0:
            ...
            # Option C: Options A/B both depend on surplus solar or existing
            # hour-23 discharge, either of which can be zero (e.g. a
            # solar-less scenario) — leaving neutrality unresolved. Last
            # resort: fund the remainder via extra grid import, honoring
            # any grid cap.
            if drift > 1e-4 and h not in no_charge:
                cur_e_after_pass = initial + sum(plan_charge) - sum(plan_discharge)
                room = round(capacity - cur_e_after_pass, 4)
                cap = grid_cap_h[h]
                grid_headroom = float("inf") if cap is None else max(0.0, cap - plan_grid[h])
                extra = min(drift, max_charge_per_h - plan_charge[h], room, grid_headroom)
                if extra > 1e-4:
                    plan_charge[h] = round(plan_charge[h] + extra, 4)
